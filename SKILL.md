...
4. **排完一篇就立刻設內鏈（不要拖到事後批次）**：對剛排程的每篇呼叫 [internal-linking] 做「單篇 outbound」——`matrix --outbound-src <本篇> --ids <同叢集已上線姊妹...>` → `interlink.js` → `apply`，把它連到同叢集**已上線**的相關文（避免連未上線→404）。連往比它晚上線的姊妹文等它自己上線時再補。
5. 到點 CMS 自動上線 → **觸發該篇「單篇 full mesh」內鏈**（`matrix --ids <本篇,已上線姊妹> --mesh`，補 inbound：姊妹文 → 本篇）。事後用 GSC/GA4 回拉成效作為下一輪重排依據（見 content-pipeline 的 backfill）。
